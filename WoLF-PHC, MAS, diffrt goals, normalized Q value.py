...
def wolf_phc_learning(position, goal_state):
    ...
    step = 0
    normalized_Q.append(math.sqrt(np.sum((Q_matrix ** 2))))
    while step != 1000:
        step += 1
        ...
        normalized_Q.append(Q_value)
    print 'reached goal!'
    print 'total steps = ', step
    return Q_matrix, pi_matrix, pi_mean_matrix, normalized_Q
...
Agent1_normalized_Q = np.asarray(Agent1_trainingResult[3])
Agent2_normalized_Q = np.asarray(Agent2_trainingResult[3])
Agent3_normalized_Q = np.asarray(Agent3_trainingResult[3])
Agent4_normalized_Q = np.asarray(Agent4_trainingResult[3])
Agent5_normalized_Q = np.asarray(Agent5_trainingResult[3])

x = np.arange(1001)
plt.plot(x, Agent1_normalized_Q, label="Agent1")
plt.plot(x, Agent2_normalized_Q, label="Agent2")
plt.plot(x, Agent3_normalized_Q, label="Agent3")
plt.plot(x, Agent4_normalized_Q, label="Agent4")
plt.plot(x, Agent5_normalized_Q, label="Agent5")
plt.legend()
plt.title("WoLF-PHC MAS, normalized Q value")
plt.show()
