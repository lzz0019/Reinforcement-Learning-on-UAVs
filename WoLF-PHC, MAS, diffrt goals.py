...
def wolf_phc_learning(position, goal_state):
    ...
    # print 'xArray= ', xArray
    # print 'yArray= ', yArray
    return Q_matrix, pi_matrix, pi_mean_matrix, xArray, yArray, timestamp
...
# print "Agent5 end with (", 79, ",", 81, ")"

Agent1_timestamp = Agent1_trainingResult[5]
Agent2_timestamp = Agent2_trainingResult[5]
Agent3_timestamp = Agent3_trainingResult[5]
Agent4_timestamp = Agent4_trainingResult[5]
Agent5_timestamp = Agent5_trainingResult[5]
...
total_timestamp = {}
for key in Agent1_timestamp:
    total_timestamp[key] = []
    total_timestamp[key].append(Agent1_timestamp[key])
    total_timestamp[key].append(Agent2_timestamp[key])
    total_timestamp[key].append(Agent3_timestamp[key])
    total_timestamp[key].append(Agent4_timestamp[key])
    total_timestamp[key].append(Agent5_timestamp[key])

for key in total_timestamp:
    states_at_step = total_timestamp[key]
    if len(states_at_step) != len(set(states_at_step)):
        print "conflicts exist in MAS trajectories at step = ", key
